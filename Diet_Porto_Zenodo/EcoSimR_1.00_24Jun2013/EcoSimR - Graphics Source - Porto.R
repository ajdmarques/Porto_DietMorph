...
plot(rep(1:ncol(Data),times = nrow(Data)),
     ...
     xlab=" ",
     ylab="Population", # define groups as population
     cex=10*sqrt(t(Data)/pi),
     col="black", # use colour to differentiate patch proximity to urban center
     lwd=2,
     yaxt="n", xaxt="n", # hide default tick marks
     main="Observed Utilization Matrix",col.main="black",cex.main=1)
axis(2, at = seq(1:8),cex.axis = 0.5,las=1, # add custom tick marks for populations
     labels=c("MAL","MIN","PAV","CID","POR","PRL","RAM","MAT"))
axis(1, at = seq(1:21),cex.axis = 0.5,las=2, # add custom tick marks for prey
     labels=c("Armadillidium","Lumbricus","Allolobophora","Eluma","Nalassus","Milax","Nemobius","Polydesmus","Porcellio","Opogona","Aporrectodea","Testacella","Chrysolina","Cochlicella","Dendrobaena","Dysdera","Phyto","Stevenia","Tipula","Atypus","Cochlicopa"))
